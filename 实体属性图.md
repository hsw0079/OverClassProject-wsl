...
> 基于 `apps/vehicle_mgmt/models.py` 中 6 个 Django 模型
...
classDiagram
    ...
    VisitorAppointment --> User : user (FK, nullable)
    QRPassToken --> VisitorAppointment : appointment (OneToOne)
```
...
| # | 字段名 | Django 类型 | 中文名 | 约束 |
|---|--------|------------|--------|------|
| 1 | `user` | `FK(auth.User)` | 关联用户 | nullable, SET_NULL |
| 2 | `visitor_name` | `CharField(50)` | 访客姓名 | NOT NULL |
| 3 | `visitor_phone` | `CharField(20)` | 访客电话 | NOT NULL |
| 4 | `plate_number` | `CharField(20)` | 车牌号 | NOT NULL |
| 5 | `host_name` | `CharField(50)` | 被访人 | NOT NULL |
| 6 | `host_department` | `CharField(100)` | 被访部门 | nullable |
| 7 | `expected_time` | `DateTimeField` | 预计进校时间 | NOT NULL |
| 8 | `purpose` | `TextField` | 事由 | NOT NULL |
| 9 | `status` | `CharField(20)` | 状态 | choices: 待审批/已通过/已拒绝/已到访/已离开 |
| 10 | `approver` | `CharField(50)` | 审批人 | nullable |
| 11 | `remarks` | `TextField` | 备注 | nullable |
| 12 | `created_at` | `DateTimeField` | 创建时间 | auto_now_add |
| 13 | `updated_at` | `DateTimeField` | 更新时间 | auto_now |
...
### 6. 二维码通行令牌 `QRPassToken`

| # | 字段名 | Django 类型 | 中文名 | 约束 |
|---|--------|------------|--------|------|
| 1 | `appointment` | `OneToOneField(VisitorAppointment)` | 关联预约 | NOT NULL, CASCADE, 反向名 `qr_token` |
| 2 | `token` | `CharField(64)` | 令牌 | **PK**, UNIQUE |
| 3 | `expires_at` | `DateTimeField` | 有效期至 | NOT NULL（生成时 +24h） |
| 4 | `used_at` | `DateTimeField` | 核销时间 | nullable |
| 5 | `created_at` | `DateTimeField` | 创建时间 | auto_now_add |

**生成逻辑**: `generate_for_appointment()` 为已通过审批的预约生成 24 小时有效的随机令牌；`is_valid()` 判断未核销且未过期。
...
└─────────────────────┘
     ...
     ● VisitorAppointment (访客预约) — 独立于车辆，FK → auth.User (user, nullable)
     ● NonMotorVehicle (非机动车档案) — 独立实体，无 FK
     ● QRPassToken (通行令牌) — OneToOne → VisitorAppointment (appointment)
```
...
|------|---------|-----|-----|------|------|------|
| SchoolVehicle | 11 | 1 | 0 | 4 | 5 | 2 |
| ExternalVehicle | 4 | 1 | 0 | 2 | 1 | 1 |
| EntryExitRecord | 14 | 0 | 2 | 5 | 8 | 1 |
| VisitorAppointment | 13 | 0 | 1 | 7 | 4 | 2 |
| NonMotorVehicle | 9 | 1 | 0 | 3 | 4 | 2 |
| QRPassToken | 5 | 1 | 1 | 3 | 1 | 1 |
| **合计** | **56** | **4** | **4** | **24** | **23** | **9** |
